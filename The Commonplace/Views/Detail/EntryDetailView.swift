...
struct EntryDetailView: View {
    ...
    var body: some View {
        ...
        .onAppear {
            DispatchQueue.main.asyncAfter(deadline: .now() + 0.5) {
                ...
                } else if (entry.type == .link || entry.type == .music) && (entry.url == nil || entry.url?.isEmpty == true) {
                    linkFieldFocused = true
                }
            }
            if entry.type == .photo && entry.extractedText == nil && entry.imagePath != nil {
                isAnalyzing = true
                Task {
                    guard let path = entry.imagePath,
                          let imageData = MediaFileManager.load(path: path) else { isAnalyzing = false; return }
        ...
        if entry.type == .music {
            if entry.url == nil || entry.url?.isEmpty == true {
                TextField("Paste Apple Music link...", text: $linkURLText)
                    ...
                    .focused($linkFieldFocused)
                    .padding(12)
                    .background((isInkwell ? entryAccentColor : Color.red).opacity(0.08))
                    .clipShape(RoundedRectangle(cornerRadius: 10))
                    .onChange(of: linkURLText) { _, newValue in
                ...
                if let urlString = entry.url, let url = URL(string: urlString) {
                    ...
                    .buttonStyle(.borderedProminent)
                    .tint(isInkwell ? entryAccentColor : .red)
                }
            }
        }
    }
    ...
    func saveMusicURL() {
        ...
        isExtracting = true
        Task {
            // Fetch basic link preview for title only
            let fetcher = await LinkPreviewFetcher()
            await fetcher.fetch(urlString: urlString)
            if let metadata = await fetcher.metadata {
                entry.linkTitle = metadata.title
            ...
            // Hit iTunes Search API for artist, album, preview URL
               ...
               let apiURL = URL(string: "https://itunes.apple.com/search?term=\(searchTerm)&limit=1&entity=song") {
                if let (apiData, _) = try? await URLSession.shared.data(from: apiURL),
                   ...
                   let first = results.first {
                    ...
                    // Fetch clean artwork directly from iTunes
