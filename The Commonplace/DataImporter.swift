    ...
    static func importArchive(from url: URL, modelContext: ModelContext) throws -> ImportResult {
        ...
        for dto in manifest.entries {
            ...
            if let filename = dto.imageFile {
                entry.imageData = try? Data(contentsOf: mediaDir.appendingPathComponent(filename))
            }
            if let filename = dto.audioFile {
                entry.audioData = try? Data(contentsOf: mediaDir.appendingPathComponent(filename))
            }
            if let filename = dto.previewImageFile {
                entry.previewImageData = try? Data(contentsOf: mediaDir.appendingPathComponent(filename))
            }
            if let filename = dto.faviconFile {
                entry.faviconData = try? Data(contentsOf: mediaDir.appendingPathComponent(filename))
            }
