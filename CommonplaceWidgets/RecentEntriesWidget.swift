...
struct RecentEntriesEntry: TimelineEntry {
    ...
    static var placeholder: RecentEntriesEntry {
        RecentEntriesEntry(
            date: Date(),
            snapshots: [
                WidgetEntrySnapshot(id: "1", type: "text",     text: "Started reading a fascinating book about the history of maps.", title: nil,            createdAt: Date(),                                              icon: "text.alignleft"),
                WidgetEntrySnapshot(id: "2", type: "music",    text: "",                                                               title: "Radiohead — OK Computer", createdAt: Date().addingTimeInterval(-3600),   icon: "music.note"),
                WidgetEntrySnapshot(id: "3", type: "location", text: "",                                                               title: "Griffith Observatory",    createdAt: Date().addingTimeInterval(-7200),   icon: "mappin.circle.fill"),
                WidgetEntrySnapshot(id: "4", type: "link",     text: "",                                                               title: "The Art of Noticing",     createdAt: Date().addingTimeInterval(-10800),  icon: "link"),
                WidgetEntrySnapshot(id: "5", type: "photo",    text: "",                                                               title: nil,                        createdAt: Date().addingTimeInterval(-14400),  icon: "photo.fill"),
                WidgetEntrySnapshot(id: "6", type: "audio",    text: "Voice memo from the drive home.",                               title: nil,                        createdAt: Date().addingTimeInterval(-18000),  icon: "waveform"),
            ]
        )
    }
}
        ...
        Group {
            if entry.snapshots.isEmpty {
                ...
                VStack(alignment: .leading, spacing: 0) {
                    widgetHeader
                    Spacer(minLength: 0)
                    ForEach(Array(entry.snapshots.prefix(3).enumerated()), id: \.element.id) { i, snapshot in
                        entryRow(snapshot: snapshot)
                        if i < min(2, entry.snapshots.prefix(3).count - 1) {
                            Divider().opacity(0.3)
                        }
                    }
                }
